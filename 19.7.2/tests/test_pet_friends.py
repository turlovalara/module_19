...
def test_get_api_key_valid_user(email=valid_email, password=valid_password):
    '''Проверяем, что код статуса запроса 200 и переменная result содержит слово key'''
...
def test_add_pets_with_valid_data(name='Vasya', animal_type='cat', age='3', pet_photo='images/Vasya.jpg'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pets(api_key, 'Milli', 'cat', '5', 'images/Milli.JPG')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_add_pets_with_valid_data_without_photo(name='КотэБезФотэ', animal_type='котей', age='7'):
...
def test_add_photo_at_pet(pet_photo='images/Fufik.jpg'):
...
def test_add_pet_negative_age_number(name='Fedor', animal_type='cat', age='-3', pet_photo='images/Fedor.jpg'):
    '''Проверка негативного сценария. Добавление питомца с отрицательным числом в переменной age.
...
def test_add_pet_with_four_digit_age_number(name='Fedor', animal_type='cat', age='1234', pet_photo='images/Vanya.jpg'):
    '''Проверка с негативным сценарием. Добавление питомца с числом более трех знаков в переменной age.
    Тест не будет пройден, еcли питомец будет добавлен на сайт с числом, превышающим три знака в поле Bозраст.'''
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='cat', age='2', pet_photo='images/Vanya.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='cat', age='2', pet_photo='images/Vanya.jpg'):
...
def test_add_pet_with_special_characters_in_variable_animal_type(name='Vanya', age='3', pet_photo='images/Vanya.jpg'):
    '''Проверка с негативным сценарием. Добавление питомца сo специальными символами вместо букв в переменной animal_type.
    Тест не будет пройден если питомец будет добавлен на сайт сo специальными символами вместо букв в поле Порода.
    '''
    animal_type = 'Kat%@'
    symbols = '#$%^&*{}|?/><=+_~@'
    symbol = []
...
def test_add_pet_with_numbers_in_variable_animal_type(name='Vanya', animal_type='34562', age='3', pet_photo='images/Vanya.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='Vanya', age='2', pet_photo='images/Vanya.jpg'):
    ...
    animal_type = 'кто ты вообще за существо и как тебя земля носит'
